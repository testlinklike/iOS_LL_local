...
この文書は、何も導入していないiPadから始めて、次の状態まで進むための手順書です。
ただし、**自分で正当に入手した復号済みiOS版リンクラ5.0.1 IPAを、既に所持して
いる人**を対象にしています。IPAの入手・復号方法は、この文書の対象外です。

- 自分で正当に入手した復号済みiOS版リンクラ5.0.1を、HASU APIへ接続する
  5.1.0互換IPAへ変換する
...
IPAは著作権で保護されたアプリ本体です。改変済みIPAを他人へ渡したり公開したり
せず、**各利用者が正当に入手した復号済みiOS版5.0.1 IPAを、自分のMacで変換**
してください。

この手順はAndroid APKをiOS IPAへ変換するものではありません。APKとIPAは
実行形式が異なるため、Android版APKだけでは作成できません。

### 0.3 必要なのは「復号済み」「ネイティブ5.0.1」のIPA

LinkuraVisualsIOS 0.8.1が対応するのは、ネイティブバイナリの実体が5.0.1で、
API向けのクライアント表示だけを5.1.0へ合わせたIPAです。

このガイドは、利用者が自分の権利の範囲で用意した復号済みIPAを既に持っている
ところから始まります。DRMや暗号化の回避、第三者のIPA入手、共有IPAの利用は
案内しません。条件を満たすIPAがない場合は、ここで作業を止めてください。
...
7. iPadOS 26では、同じフォルダへ対応する `dobby.dylib` も入れます。
...
5. iPadOS 26では `dobby.dylib` が残っていることを確認します。
6. 全ファイルのスイッチをオンにします。
7. JITを取り直して起動します。
...
3. フォルダに次がある
   ...
   - `LinkuraVisualsIOS.json`
   - `IL2CPP_OFFSETS_5.0.1.json`
   - iPadOS 26なら `dobby.dylib`
4. 調整画面の全ファイルがオン
5. `JITで起動` がオン
6. iPadOS 26ならJIT起動スクリプトが `Geode.js`
...
- [ ] iPadOS 26では、信頼できるDobby／Geode.jsの入手元・版・SHA-256を記録した
- [ ] iPadOS 26ではDobbyを追加した
- [ ] iPadOS 26ではJIT起動スクリプトにGeode.jsを選んだ
